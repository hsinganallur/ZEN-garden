...
def get_data_for_percentage(data, percentage):
    ...
    battery_energy = data_percent['battery_energy(TWh)'].values
    hydrogen_energy = data_percent['hydrogen_storage_energy(TWh)'].values
    up_devices_energy = data_percent['up_devices_energy(TWh)'].values
    pumped_hydro_energy = data_percent['pumped_hydro_storage_energy(TWh)'].values
...
def plot_and_save_image(common_discharge_times, included_energies, included_labels, percentage, folder_path):
    ...
    # Customize the plot
    #plt.title(f'Energy (TWh) vs. Discharge Times (h) for {percentage}% Total Costs by Technology')
    plt.xlabel('Discharge Time (hours)', fontsize=15, fontweight='bold')
    plt.ylabel('Energy (TWh)', fontsize=15, fontweight='bold')
    plt.ylim(0, 50)

    # Set the x-axis limits and labels from 10 to 150 with steps of 10
    ...
    plot_file_path = os.path.join(folder_path, f'energy_plot_{percentage}.png')
    plt.tight_layout()
...
def save_data_to_excel(common_discharge_times, included_energies, included_labels, percentage, folder_path):
    ...
    for label, energy in zip(included_labels, included_energies):
        df[label + ' (TWh)'] = energy
    data_file_path = os.path.join(folder_path, f'energy_data_{percentage}.xlsx')
...
def main():
    ...
    file_path = "C:\\Users\\Hareesh S P\\Documents\\MT_Offline\\MT_Simulations\\00_Extreme_Pessimistic\\Results\\Energy_TWh_Upload.xlsx"  # Replace with your actual file path
