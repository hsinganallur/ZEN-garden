...
def generate_plots(file_path, save_path):
    ...
    for tech, power_df in power_data.items():
        ...
        plt.savefig(f"{save_path}/{tech}_installed_power.png", bbox_inches='tight')  # Save plot with tight bounding box
    ...
    for tech, energy_df in energy_data.items():
        ...
        ax.set_ylim(top=None)  # Remove the upper limit of y-axis
        plt.tight_layout()  # Adjust layout
        plt.savefig(f"{save_path}/{tech}_installed_energy.png", bbox_inches='tight')  # Save plot with tight bounding box
