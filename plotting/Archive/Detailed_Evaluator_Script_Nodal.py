...
base_folder = "C:\\Users\\Hareesh S P\\OneDrive - Unbound Potential GmbH\\MasterThesis\\Simulations\\T_19\\Results"
...
technologies_to_keep = ['biomass_plant_CCS', 'biomass_plant', 'hard_coal_plant',
                        'hard_coal_plant_CCS', 'lignite_coal_plant', 'natural_gas_turbine',
                        'natural_gas_turbine_CCS', 'nuclear', 'oil_plant', 'photovoltaics',
                        'reservoir_hydro', 'run-of-river_hydro', 'waste_plant', 'wind_offshore',
                        'wind_onshore', 'battery', 'hydrogen_storage', 'pumped_hydro', 'vanadium_redox_flow_battery',
                        "up_redox_flow_battery_1", "up_redox_flow_battery_2", "up_redox_flow_battery_3",
                        "up_redox_flow_battery_4", "up_redox_flow_battery_5"]
...
data_1_power_filtered.columns = ['2024']
# Sort the data by the summed value in ascending order
data_1_power_filtered_sorted = data_1_power_filtered.sort_values(by='2024', ascending=True)
...
# Rename the second column to 'summed_value'
data_1_energy_filtered.columns = ['2024']
# Sort the data by the summed value in ascending order
data_1_energy_filtered_sorted = data_1_energy_filtered.sort_values(by='2024', ascending=True)
...
cumulative_data.plot(kind='area', figsize=(14, 8), linewidth=2, color=[color_map.get(x, 'gray') for x in cumulative_data.columns])
# Adjusting Y-axis limits
plt.ylim(0, 0.8)
# Set plot labels and title
plt.xlabel('Time')
plt.ylabel('Power (TW)')
plt.title('Stacked Line Plots of Power Flows for All Technologies Over Time')
...
# Data for plotting
categories = ['Capex', 'Opex', 'Carbon Emissions', 'Carrier']
colors = ['blue', 'green', 'red', 'purple']

# Plotting
plt.figure(figsize=(10, 6))

# Stacked bar for Capex, Opex, Carbon Emissions, Carrier
bottom = 0
for i, cost in enumerate(costs[:4]):
    plt.bar('Stacked Costs', cost, bottom=bottom, color=colors[i], label=categories[i])
    bottom += cost

# Separate bar for Total Costs
plt.bar('Total Costs', costs[4], color='grey', label='Total Costs')
...
# Save the plot
output_path = out_folder1 + "\\Costs_plot.png"
plt.savefig(output_path, bbox_inches='tight')

# Shed Demand
data_shed_demand = r.get_total("cost_shed_demand").groupby('carrier').sum()
data_shed_demand = data_shed_demand[(data_shed_demand.iloc[:, 1:] <= 1).any(axis=1)].dropna()
...
plt.bar(carriers, costs, color=[color_map.get(x, 'gray') for x in carriers])
plt.xlabel('Carriers')
...
# Save the plot
output_path = out_folder1 + "\\Shed_Demand_plot.png"
plt.savefig(output_path)
